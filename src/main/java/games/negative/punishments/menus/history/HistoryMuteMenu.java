...
import games.negative.punishments.menus.modify.ModifyMutePunishmentMenu;
...
public class HistoryMuteMenu extends GUI {
    public HistoryMuteMenu(Player staff, PersistentOfflinePlayer offender, int page) {
        super("&6&lViewing Mutes of &e&l" + offender.getName(), 5);
        ...
        long limit = 27;
        Collection<PersistentPunishment> raw = DeltaPunishments.getInstance().getCacheManager().getPunishments(offender.getUniqueID(), PunishmentType.MUTE);
        LinkedList<PersistentPunishment> mutes = raw.stream()
                .sorted(Comparator.comparingLong(PersistentPunishment::getTime).reversed())
                .skip((page - 1) * limit).limit(limit)
                .collect(Collectors.toCollection(LinkedList::new));

        for (PersistentPunishment punishment : mutes) {
            ...
            lore.add(" ");
            lore.add("&7Muted by: &f" + bannedBy);
            lore.add("&7Reason: &c" + punishment.getReason());
            lore.add(" ");
            lore.add("&7Original Mute Date");
            lore.add("&f" + originFormatted);
            lore.add(" ");
            lore.add("&7Mute Removal Date");
            ...
            addItemClickEvent(player -> record, (player, event) -> {
                ...
                new ModifyMutePunishmentMenu(punishment).open(player);
            });
        }

        if (raw.size() > page * limit) {
            ItemStack next = new ItemBuilder(Material.ARROW).setName("&aNext Page").build();
            setItemClickEvent(44, player -> next, (player, event) ->
                    new HistoryMuteMenu(staff, offender, page + 1).open(player));
        }
