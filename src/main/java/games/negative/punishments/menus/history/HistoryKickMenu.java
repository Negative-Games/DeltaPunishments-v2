...
import games.negative.punishments.menus.modify.ModifyKickPunishmentMenu;
...
public class HistoryKickMenu extends GUI {
    public HistoryKickMenu(Player staff, PersistentOfflinePlayer offender, int page) {
        super("&6&lViewing Kicks of &e&l" + offender.getName(), 5);
        ...
        Collection<PersistentPunishment> raw = DeltaPunishments.getInstance().getCacheManager().getPunishments(offender.getUniqueID(), PunishmentType.KICK);
        LinkedList<PersistentPunishment> kicks = raw.stream()
                .sorted(Comparator.comparingLong(PersistentPunishment::getTime).reversed())
                .skip((page - 1) * limit).limit(limit)
                .collect(Collectors.toCollection(LinkedList::new));

        for (PersistentPunishment punishment : kicks) {
            ...
            lore.add(" ");
            lore.add("&7Kicked by: &f" + bannedBy);
            lore.add("&7Reason: &c" + punishment.getReason());
            lore.add(" ");
            lore.add("&7Original Kick Date");
            ...
            addItemClickEvent(player -> record, (player, event) -> {
                ...
                new ModifyKickPunishmentMenu(punishment).open(player);
            });
        }

        if (kicks.size() > page * limit) {
            ItemStack next = new ItemBuilder(Material.ARROW).setName("&aNext Page").build();
            setItemClickEvent(44, player -> next, (player, event) ->
                    new HistoryKickMenu(staff, offender, page + 1).open(player));
        }
